...
{
  uint8_t buff[2];
  int result = fread(buff, sizeof(uint8_t), 2, f);
  *i = *((uint16_t *)buff);
  return result;
}
...
{
  uint8_t buff[4];
  int result = fread(buff, sizeof(uint8_t), 4, f);
  *i = *((uint32_t *)buff);
  return result;
}
...
{
  uint8_t val[2] = {((uint8_t *)i)[0], ((uint8_t *)i)[1]};
  return fwrite((const void *)val, sizeof(uint8_t), 2, f);
}

int writeInt32(const int32_t *i, FILE *f)
{
  int8_t val[4] = {((uint8_t *)i)[0], ((uint8_t *)i)[1], ((uint8_t *)i)[2], ((uint8_t *)i)[3]};
  return fwrite((const void *)&val, sizeof(uint8_t), 4, f);
}

int writeDouble(const double *d, FILE *f)
{
  int arr_length = sizeof(double);
  uint8_t val[arr_length];
  for (int i = 0; i < arr_length; i++)
  {
    val[i] = ((uint8_t *)d)[i];
  }
  return fwrite((const void *)&val, sizeof(uint8_t), arr_length, f);
}
...
int storeLocalmotionBinary(FILE *f, const LocalMotion *lm)
{
  if (writeInt16(&lm->v.x, f) <= 0)
    return 0;
  if (writeInt16(&lm->v.y, f) <= 0)
    return 0;
  if (writeInt16(&lm->f.x, f) <= 0)
    return 0;
  if (writeInt16(&lm->f.y, f) <= 0)
    return 0;
  if (writeInt16(&lm->f.size, f) <= 0)
    return 0;
  if (writeDouble(&lm->contrast, f) <= 0)
    return 0;
  if (writeDouble(&lm->match, f) <= 0)
    return 0;
  return 1;
}
...
LocalMotion restoreLocalmotionBinary(FILE *f)
{
  ...
  if (readInt16(&lm.v.x, f) <= 0)
    goto parse_error_handling;
  if (readInt16(&lm.v.y, f) <= 0)
    goto parse_error_handling;
  if (readInt16(&lm.f.x, f) <= 0)
    goto parse_error_handling;
  if (readInt16(&lm.f.y, f) <= 0)
    goto parse_error_handling;
  if (readInt16(&lm.f.size, f) <= 0)
    goto parse_error_handling;
  if (readDouble(&lm.contrast, f) <= 0)
    goto parse_error_handling;
  if (readDouble(&lm.match, f) <= 0)
    goto parse_error_handling;
...
int vsStoreLocalmotionsText(FILE *f, const LocalMotions *lms)
{
  ...
  for (i = 0; i < len; i++)
  {
    if (i > 0)
      fprintf(f, ",");
    if (storeLocalmotion(f, LMGet(lms, i), ASCII_SERIALIZATION_MODE) <= 0)
      return 0;
  }
  fprintf(f, "]");
  return 1;
...
int vsStoreLocalmotionsBinary(FILE *f, const LocalMotions *lms)
{
  ...
  int i;
  if (writeInt32(&len, f) <= 0)
    return 0;
  for (i = 0; i < len; i++)
  {
    if (storeLocalmotion(f, LMGet(lms, i), BINARY_SERIALIZATION_MODE) <= 0)
      return 0;
  }
  return 1;
}
...
int vsWriteToFileText(const VSMotionDetect *md, FILE *f, const LocalMotions *lms)
{
  ...
  if (fprintf(f, "Frame %i (", md->frameNum) > 0 && vsStoreLocalmotions(f, lms, ASCII_SERIALIZATION_MODE) > 0 && fprintf(f, ")\n"))
    return VS_OK;
  else
    return VS_ERROR;
}
...
int vsWriteToFileBinary(const VSMotionDetect *md, FILE *f, const LocalMotions *lms)
{
  ...
  if (writeInt32(&md->frameNum, f) <= 0)
    return VS_ERROR;
  if (vsStoreLocalmotions(f, lms, BINARY_SERIALIZATION_MODE) <= 0)
    return VS_ERROR;
...
int vsGuessSerializationMode(FILE *f)
{
  ...
  if (fgetc(f) == 'T' && fgetc(f) == 'R' && fgetc(f) == 'F')
  {
...
int vsReadFileVersionBinary(FILE *f)
{
  ...
  if (fscanf(f, "TRF%hhu\n", &version) != LIBVIDSTAB_FILE_FORMAT_VERSION)
    goto parse_error_handling;
  if (readInt32(&conf.accuracy, f) <= 0)
    goto parse_error_handling;
  if (readInt32(&conf.shakiness, f) <= 0)
    goto parse_error_handling;
  if (readInt32(&conf.stepSize, f) <= 0)
    goto parse_error_handling;
  if (readDouble(&conf.contrastThreshold, f) <= 0)
    goto parse_error_handling;
